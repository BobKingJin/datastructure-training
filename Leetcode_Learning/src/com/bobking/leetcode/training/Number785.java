...
/**
 * @author BobKing
 * @create 2022-08-06 11:49
 */
public class Number785 {

    // 参考：https://leetcode.cn/problems/is-graph-bipartite/solution/bfs-dfs-bing-cha-ji-san-chong-fang-fa-pan-duan-er-/
    public boolean isBipartite1(int[][] graph) {
        ...
        // 因为图中可能含有多个连通域，所以需要判断是否存在顶点未被访问，若存在则从它开始再进行一轮 bfs 染色
    ...
    public boolean isBipartite2(int[][] graph) {
        ...
        // 定义 visited 数组，初始值为 0 表示未被访问，赋值为 1 或者 -1 表示两种不同的颜色
        int[] visited = new int[graph.length];
        // 因为图中可能含有多个连通域，所以需要判断是否存在顶点未被访问，若存在则从它开始再进行一轮 dfs 染色
        for (int i = 0; i < graph.length; i++) {
