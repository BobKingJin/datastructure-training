...
public class Number79 {
    ...
    private boolean backTrace2(char[][] board, String word, int row, int column, int index,
        boolean[][] marked) {
        ...
        if (board[row][column] == word.charAt(index)) {
            marked[row][column] = true;
            if (isLegal(row - 1, column, board.length - 1, board[0].length - 1) && !marked[row
                - 1][column]) {
                if (backTrace2(board, word, row - 1, column, index + 1, marked)) {
                    return true;
                }
            }
            if (isLegal(row, column - 1, board.length - 1, board[0].length - 1) && !marked[row][
                column - 1]) {
